...
const PingPongModels = require("../bot/ping-pong-models");
...
test('cachifyPromiser', utils_1.loudAsync((t) => __awaiter(this, void 0, void 0, function* () {
    const actions = [
        () => __awaiter(this, void 0, void 0, function* () {
    ...
    let i = 0;
    const fn = utils_1.cachifyPromiser(() => actions[i++]());
    try {
        yield fn();
        t.fail('expected error');
    }
...
test('batch by size', function (t) {
    ...
    t.same(utils_1.batchByByteLength(input, MAX), expected);
    t.end();
});
...
test('content-addressed-storage', utils_1.loudAsync((t) => __awaiter(this, void 0, void 0, function* () {
    ...
    const key = yield contentAddressedStore.put('a');
...
test('key-value table', utils_1.loudAsync((t) => __awaiter(this, void 0, void 0, function* () {
    ...
    const { aws } = tradle;
    yield aws.dynamodb.createTable(Object.assign({}, KVTable.Properties, { TableName: newTableName })).promise();
...
test('first success', utils_1.loudAsync((t) => __awaiter(this, void 0, void 0, function* () {
    const pending = [
        ...
        utils_1.timeoutIn({ millis: 150 })
    ];
    const failed = [
        utils_1.timeoutIn({ millis: 0 }),
        utils_1.timeoutIn({ millis: 50 })
    ];
    const resolved = [
...
test('ModelStore', utils_1.loudAsync((t) => __awaiter(this, void 0, void 0, function* () {
    const friend1 = {
    ...
    const store = model_store_1.createModelStore(tradle);
    sinon.stub(tradle.friends, 'getByDomain').callsFake((domain) => __awaiter(this, void 0, void 0, function* () {
        if (domain === friend1.domain)
            return friend1;
        if (domain === friend2.domain)
            return friend2;
        throw new Errors.NotFound(`friend for domain: ${domain}`);
    }));
    try {
        yield store.getModelsPackByDomain(friend1.domain);
        t.fail('expected error');
    }
    catch (err) {
    ...
    let memBucket = {};
    sinon.stub(store.bucket, 'gzipAndPut').callsFake((key, value) => __awaiter(this, void 0, void 0, function* () {
        memBucket[key] = value;
    }));
    sinon.stub(store.bucket, 'getJSON').callsFake((key) => __awaiter(this, void 0, void 0, function* () {
        if (!(key in memBucket)) {
    ...
    try {
        yield store.saveModelsPack({ modelsPack });
        t.fail('expected error');
    }
    catch (err) {
    ...
    yield store.saveModelsPack({ modelsPack });
    t.same(yield store.getModelsPackByDomain(friend1.domain), modelsPack);
    t.equal(yield store.getCumulativeModelsPack(), null);
    store.setCustomModels({ namespace: 'ping.pong', models: PingPongModels });
    t.equal(yield store.getCumulativeModelsPack(), null);
    console.log('patience...');
    yield store.addModelsPack({ modelsPack });
    t.same(yield store.getCumulativeModelsPack(), _.omit(modelsPack, 'namespace'), 'models pack added to cumulative pack');
    yield store.saveCustomModels({ namespace: 'ping.pong', models: PingPongModels });
    let cumulative = yield store.getCumulativeModelsPack();
    ...
    try {
        yield store.addModelsPack({
            modelsPack: Object.assign({}, modelsPack2, { namespace })
        });
    ...
    console.log('patience...');
    yield store.addModelsPack({ modelsPack: modelsPack2 });
    cumulative = yield store.getCumulativeModelsPack();
